---
name: spec-quality-review
description: spec PR を人間がレビューする前に、Kiro spec の requirements.md と design.md の記述品質、設計理由、development-rules.md への準拠を確認するとき、または同じレビューで投稿した指摘への Kiro の対応を再確認するときに使う。
---

# Spec 品質レビュー

spec PR を人間がレビューする前に、対象 spec 一覧に含まれる各 spec の `requirements.md` と `design.md` がプロジェクトの spec 運用ルールに沿っているか確認する。
対象 spec が 1 件だけでも、依頼では必ずリストとして列挙される前提で読む。

## モード判定

- 未レビューの PR または通常のレビュー依頼では「初回レビューモード」を実行する
- 「レビュー指摘対応を再確認」と依頼された場合、または同じ PR にこのスキルの識別子付き指摘がある場合は「指摘対応再確認モード」を実行する

## 読むファイル

1. 対象 spec 一覧に含まれる各 `.kiro/specs/<spec名>/requirements.md`
2. 対象 spec 一覧に含まれる各 `.kiro/specs/<spec名>/design.md`
3. `.kiro/steering/development-rules.md`
4. 必要に応じて `docs/requirements-spec.md`

## 確認観点

- `requirements.md` にユーザーストーリーと受け入れ基準が明確に書かれているか
- `design.md` に、その spec 内で必要なインターフェース・コマンド・データ構造だけが自己完結して書かれているか
- `design.md` に「なぜその設計にしたか」が読み取れる設計判断の理由があるか
- 全体アーキテクチャや共通データモデルなど、`docs/requirements-spec.md` に置くべき内容が混ざっていないか
- requirements と design の間に矛盾、曖昧さ、未定義の用語がないか
- 複数 spec がある場合、spec 間に矛盾や責務の重複がないか

## 出力形式

レビュー結果は、次の順で簡潔に出す。

1. **指摘事項**: 重大度順に、ファイルパスと該当箇所を添えて書く
2. **確認できたこと**: 問題がなかった主要観点を短く書く
3. **提案**: 人間または Kiro に依頼すべき修正があれば具体的に書く
4. **Kiro への依頼プロンプト**: 修正が必要な場合、Kiro にそのまま渡せるプロンプトをコードブロックで書く
5. **マージ後の Kiro 依頼プロンプト**: spec PR をマージしてよい状態の場合、マージ後に Kiro へ `tasks.md` 作成を依頼するプロンプトをコードブロックで書く

問題がない場合は、指摘事項なしと明記し、残るリスクや未確認範囲だけを書く。
修正が不要な場合、**Kiro への依頼プロンプト** は「なし」と書く。
spec PR をマージしてよい状態でない場合、**マージ後の Kiro 依頼プロンプト** は「なし」と書く。
Kiro への依頼プロンプト内にコードブロックや Markdown 例を含める場合は、外側のコードフェンスを内側より長くするなど、表示が崩れない形式にする。

Kiro への依頼プロンプトには、次を含める。

- 対象 spec 名
- 修正対象ファイル
- 修正してほしい内容
- 守るべき制約や参照すべきルール
- 完了条件

マージ後の Kiro 依頼プロンプトには、実際の対象 spec ディレクトリを含める。対象 spec を確定できない場合は、プレースホルダを残さず「対象 spec 不明のため出力不可」と書く。

```text
#task-create-from-merged-spec 以下の spec PR について tasks.md を作成してください:
PR: #<PR番号またはPR URL>

対象 spec:
- `.kiro/specs/<具体的なspec名>/`
```

## PR へのコメント投稿

PR 番号または PR URL が指定されている場合は、`/github-operations` を参照してレビュー結果を対象 PR へ投稿する。PR が指定されていないローカル差分レビューでは投稿しない。

- レビュー種別の識別子には `spec-quality` を使う
- 「指摘事項」の各項目を、1 項目につき 1 件、該当ファイルの変更行へのインラインレビューコメントとして投稿する。指摘事項を通常コメントとして投稿してはならない
- 指摘ごとにスレッドを分け、PR 上で返信・解決できる状態にする
- 削除された要素、記述の欠落、複数ファイルにまたがる問題など、問題そのものを表す変更行がない場合も、最も関連する変更行を選んでインラインコメントとして投稿し、コメント本文でその行との関係と指摘対象の範囲を明記する
- 各指摘コメントには、重大度、ファイルパスと該当箇所、問題、spec 運用ルール上の根拠、修正案を含める
- 指摘投稿後に、結論、指摘件数と重大度、確認結果、マージへ進めるか、未解決事項だけを簡潔にまとめた総括コメントを 1 件、PR の通常コメントとして投稿する
- 指摘事項がない場合は、総括コメントだけを投稿し、「指摘事項なし」と明記する
- 確認できたことの詳細、提案の詳細、Kiro への依頼プロンプト、マージ後の Kiro 依頼プロンプトは PR コメントに含めず、投稿後のチャット応答にのみ出力する
- 投稿後のチャット応答には、投稿先 PR、投稿したインライン指摘コメント数、総括コメントの投稿結果を記載する
- PR の head commit SHA を取得し、各コメント末尾に次の識別子を入れる
  - 指摘: `<!-- claude-review:spec-quality:<head SHA>:finding:<連番> -->`
  - 総括: `<!-- claude-review:spec-quality:<head SHA>:summary -->`
- 投稿前に既存のインラインレビューコメントと通常コメントを確認し、同じ識別子があるコメントは再投稿しない
- 各投稿の末尾では、`claude-review` 識別子の直前に Claude Code の署名と `<!-- ai-agent:claude-code -->` を入れる

## 指摘対応再確認モード

同じ `/spec-quality-review` を再度呼び出して実行する。新しいスキルは使わない。

1. `/github-operations` を参照し、PR の最新 head commit SHA、レビューコメント、返信、レビュースレッドの解決状態を取得する
2. `<!-- claude-review:spec-quality:` を含む各未解決指摘について、Kiro の返信と署名、返信で示された commit、最新差分を対応付ける
3. 対応した指摘は、指摘箇所だけでなく関連する requirements/design、Issue の承認済み内容との整合まで再確認する。
4. 対応しない旨が返信されている指摘は、その理由をルール、Issue、spec、差分に照らして独立に評価する。指摘の採用自体を必須としない
5. 各スレッドを次のいずれかに分類する
   - `解決`: 修正が妥当、または対応しない理由が妥当
   - `継続`: 修正不足、回帰、検証不足、または対応しない理由が不十分
   - `未返信`: Kiro から判断と根拠の返信がない
6. `解決` のスレッドには再確認結果を返信してから解決する
7. `継続` のスレッドには不足内容を返信し、解決しない。新たに独立した問題を発見した場合だけ、最新 head SHA を使って新しいインライン指摘を投稿する
8. `未返信` のスレッドは解決せず、返信が必要であることを総括に記載する
9. PR の通常コメントへ、解決・継続・未返信の件数、最新検証結果、マージへ進めるかを再確認総括として投稿する
10. チャット応答には、各指摘の判定を記載する。残る指摘がある場合は Kiro へ同じ steering を再度呼び出すプロンプトを出力し、すべて解決して spec PR をマージしてよい状態の場合は、マージ後に Kiro へ `tasks.md` 作成を依頼するプロンプトを出力する

再確認返信と総括には次の識別子を使い、同じ識別子がある場合は再投稿しない。

- スレッド返信: `<!-- claude-review:spec-quality:<head SHA>:recheck:<元コメントID> -->`
- 総括: `<!-- claude-review:spec-quality:<head SHA>:recheck-summary -->`

残る指摘がある場合の Kiro 向けプロンプト:

```text
#spec-update-from-approved-issue spec PR #<PR番号> のレビュー指摘に対応してください。
```
