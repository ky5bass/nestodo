...
- **バージョン**: v1.1
- **スコープ**: タスクリスト表示（ツリー構造・プレビュー）、詳細パネル遷移、グローバルフィルター、日付境界、詳細パネルからの即時反映
...
3. タスクにpreviewが存在する場合（空文字列でない場合）、Task_List_Viewはタスク名の下にプレビューを小さいフォントサイズで表示すること
4. previewがnullまたは空文字列の場合、Task_List_Viewはプレビュー行を表示せず、プレビュー用の余白を設けないこと
5. Task_List_Viewは、タスクのPriorityに応じてタスクUIコンポーネントの背景色を変更すること：none（なし）の場合はデフォルトの背景色、priority（優先）の場合は黄色系の背景色、highest（最優先）の場合は赤色系の背景色とすること
6. Task_List_Viewは、各階層内の兄弟タスクをsort_orderの昇順で並べること
