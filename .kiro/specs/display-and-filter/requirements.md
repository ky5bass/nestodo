...
nestodoのタスクリスト画面における表示とフィルタリング機能を定義する。ツリー構造でのタスク表示、タスク状態に応じたサブテキスト表示、通常モードでのクリックによる詳細パネル遷移、およびサーバーサイドのグローバルフィルター（effective_at算出、表示条件判定、先祖包含）を対象とする。
...
3. 午前4時30分にlast_done_atが更新されたタスクについて、Filter_Serviceはそのlast_done_atを前日（直前の午前5時に開始した日）の日付として扱うこと
