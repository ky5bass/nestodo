...
4. 一括編集モード中、Edit_Mode_Controllerはタスク詳細パネルへのナビゲーション（クリックで開く）を無効化すること
...
1. 一括編集モード中にタスク追加アクションが実行された場合、Change_Bufferは一時ID、タスク名、親ID、算出されたsort_orderを含む作成操作を保存すること
...
1. 一括編集モード中、Edit_Mode_Controllerは各タスクにドラッグハンドル（4本線アイコン）を表示し、ドラッグは同一親の兄弟間に制限すること
