...
2. Batch_Completion_Modalは未完了子孫のタスク名とステータスをリスト形式で表示すること（task-management-core Requirement 6の確認要求レスポンスを使用）
...
4. task-management-core Requirement 6の受け入れ基準1について、全完了経路で同一の一括完了判定を通すこと（PUT /api/tasks/{id}でのprogress=100設定も含む）
