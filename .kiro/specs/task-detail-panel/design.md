...
export class TimeInputComponent {
  ...
  onSlider(unit: string, indexValue: string): void; // index=0で0を設定、index>=1でsteps[index-1]を設定
}
```
...
**設計判断（スライダー左端=0）**: スライダーのindex=0を「0」に割り当て、index=1以降を従来の離散値配列に対応させる設計とした。テキスト入力で0を設定できるようにした以上、スライダー操作でも同じ値に到達できないのは不自然であるため。rangeのmaxは`steps.length`（0位置分を含む）とし、`stepIndex`はvalue=0のとき0、1以上のとき`steps.indexOf(...) + 1`を返す。
