...
2. Kiro → Codex CLI non-interactive mode を呼び出し、Codex に実装・テスト・PR 更新を行わせる
3. Kiro → Claude Code を呼び出し、実装 PR の spec 整合性とコード品質を確認させる
4. 人間 → Claude Code の `承認可能` 判定後に動作確認し、マージ可否を判断する
...
### やること
- `.kiro/specs/` の spec ファイルの **記述品質チェック**（設計理由の明記・ルール準拠）。詳細は `/spec-quality-review` スキルを参照
...
Claude Code が担うのはステップ4（spec の記述品質）とステップ6（実装 PR のコード品質・spec 整合性、レビュー指摘対応の再確認）。Kiro はステップ8で spec 完了処理と Issue クローズを行う。詳細は `docs/CONTRIBUTING.md` を参照。
...
# Kiro から Codex CLI non-interactive mode を呼び出すときのコマンド例
command -v codex
