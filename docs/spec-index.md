...
| `task-detail-panel` | v1.4 | 完了 | 右サイド詳細パネル、属性編集UI改善、カレンダーピッカー、進捗スライダー、作業時間入力、export_flag、Optimistic UI、last_done_at更新制御 | Detail panel UI, DetailSaveService, task contents editing | `task-crud`, `display-and-filter` | `batch-completion-ui`, `pwa-offline`, `daily-export` |
