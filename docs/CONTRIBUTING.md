...
指摘が投稿された場合は、Kiro に同じ `#spec-update-from-approved-issue` steering を再度使って対応させ、その後 Claude Code に同じ `/spec-quality-review` を再度使って確認させる:
...
指摘が投稿された場合は、Codex に同じ `$task-implementer` を再度使って対応させ、その後 Claude Code に同じ `/spec-impl-checker` を再度使って確認させる:
...
指摘が投稿された場合は、Kiro に同じ `#spec-split` steering を再度使って対応させ、その後 Claude Code に同じ `/spec-split-completeness-review` を再度使って確認させる:
