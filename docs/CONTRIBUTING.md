...
Claude Code に PR レビューを依頼した場合、指摘事項はすべて該当ファイルの変更行にインラインのレビューコメントとして投稿する。指摘ごとにスレッドを分け、PR 上で返信・解決できる状態にする。指摘事項を PR の通常コメントとして投稿してはならない。

削除された要素、実装の欠落、複数ファイルにまたがる問題など、問題そのものを表す変更行がない指摘も、最も関連する変更行を選び、インラインレビューコメントとして投稿する。コメント本文で、選択した行との関係と指摘対象の範囲を明記する。

指摘投稿後、PR の通常コメントとして短い総括を 1 件投稿する。総括には、結論、指摘件数と重大度、テスト・検証結果、次の工程へ進めるか、未解決事項だけを記載する。指摘事項がない場合も、レビュー済みの記録として総括を投稿し、「指摘事項なし」と明記する。

Kiro、Codex などへの修正依頼プロンプトや次工程の実行プロンプトは PR コメントに含めず、レビュー依頼元のチャットにのみ出力する。

## レビュー指摘への対応と再確認

レビュー指摘は必ず採用するものではない。各指摘には対応内容または対応しない理由を返信し、Claude Code の再確認前にはスレッドを解決しない。詳細は `.agents/github-rules.md` に従う。

指摘対応と再確認には、PR 作成・実装・初回レビューで使用した既存スキルを再度使用する。

未解決の指摘がなくなるまで再確認し、最後に人間がマージ可否を判断する。

## AI エージェントが作成した GitHub リソースの表示

AI エージェントが作成する GitHub リソースの表示方法は `.agents/github-rules.md` に従う。
...
Kiro は `#spec-clarify-from-issue` スキルに従い、Issue を対話で具体化して明示的な承認を得た後、承認内容を自己完結した Issue コメントとして記録し、spec を変更せずにセッションを終了する。
...
Kiro は `#spec-update-from-approved-issue` スキルに従い、過去の会話に頼らず最新の承認済みコメントから spec PR を作成する。
...
指摘が投稿された場合は、Kiro に同じ `#spec-update-from-approved-issue` スキルを再度使って対応させ、その後 Claude Code に同じ `/spec-quality-review` スキルを再度使って確認させる:
...
spec PR #XX のレビュー指摘対応を再確認してください。
```
...
詳細な分解手順は `.kiro/skills/spec-split/SKILL.md` にまとめている。
...
指摘が投稿された場合は、Kiro に同じ `#spec-split` スキル を再度使って対応させ、その後 Claude Code に同じ `/spec-split-completeness-review` スキルを再度使って確認させる:
...
spec 分解 PR #XX のレビュー指摘対応を再確認してください。
```
...
- `.kiro/steering/development-rules.md` を常に参照して動作するため、ルール違反を自動で防げる
